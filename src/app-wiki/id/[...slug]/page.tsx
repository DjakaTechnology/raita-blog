...
export function generateStaticParams() {
  return getAllWikiSlugs("id").map((slug) => ({
    slug: slug.split("/"),
  }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string[] }> }): Promise<Metadata> {
  ...
  const page = await getWikiPage(slug, "id");
  if (!page) return {};
  return {
    title: page.title,
    description: page.description,
    alternates: { canonical: `https://raita.ai/wiki/id/${page.slug}` },
    openGraph: { title: page.title, description: page.description, type: "article", url: `https://raita.ai/wiki/id/${page.slug}` },
  };
}

export default async function WikiPageRouteID({ params }: { params: Promise<{ slug: string[] }> }) {
  const { slug } = await params;
  const page = await getWikiPage(slug, "id");
  if (!page) notFound();

  const allPages = getAllWikiPageMeta("id");
  const titles: Record<string, string> = {};
  for (const p of allPages) {
    titles[p.slug] = p.title;
  }

  const section = getWikiSection(slug.join("/"), "id");

  const pageList = getWikiPageList("id");
  ...
  return (
    <div className="max-w-[680px] mx-auto px-6 py-8">
      <WikiBreadcrumb slugParts={slug} titles={titles} basePath="/id" />
          ...
          {section.children.map((child) => (
            ...
            >
              ...
              <span className="text-xs text-muted-foreground mt-2 block">
                {child.pages.length + child.children.length} item
              </span>
            </Link>
          ))}
        </div>
      ...
      {(prev || next) && (
        <nav className="mt-12 flex items-stretch gap-4 border-t border-border pt-6">
          {prev ? (
            ...
            >
              <span className="text-xs text-muted-foreground mb-1">← Sebelumnya</span>
          ...
          {next ? (
            ...
            >
              <span className="text-xs text-muted-foreground mb-1">Selanjutnya →</span>
              <span className="text-sm font-semibold text-foreground group-hover:text-primary transition-colors">
                {next.title}
              </span>
            </Link>
